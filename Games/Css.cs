...
{
    public class Css : BaseGameServer
    {
	...
	$run = 'cd /host/'.$user.'/'.$id.'/css/;chmod 777 srcds_run;screen -dmS server_'.$id.' sudo -u s'.$id.' 
    ./srcds_run -console -game cstrike +servercfgfile server.cfg  -nohltv -autoupdate +map \''.$map.'\' 
    +maxplayers '.$slots.' +ip '.$ip2.' -port '.$port2.' '.$dop;
	$ssh->exec_cmd($run);
	sleep('2');
	...
	$ssh->exec_cmd('screen -dmS server_cpu_'.$id.' cpulimit -v -z -p '.$data.' -l '.$cpu.';');                */
                var run = $"cd /host/{userId}/{id}/css/;chmod 777 srcds_run;screen -dmS server_{id} sudo -u s{id}";
                run += $" ./srcds_run -console -game cstrike +servercfgfile server.cfg -nohltv -autoupdate +map '{map}'";
