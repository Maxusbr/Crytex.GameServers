...
{
    public class CssOld : BaseGameServer
    {
        public CssOld(ConnectParam param) : base(param) { }
        ...
        public override void On(ConnectParam param)
        {
            ...
            drop += $" +fps_max 1000 +sv_lan 0 -insecure +tv_enable 1 +tv_maxclients 1  +tv_port {tvport}";
            /*
if($row['p9'] != ""){$dop = " +sv_password '".$pass2."'";}
if($row['p14'] != ""){$dop .= " +rcon_password '".$rcon."'";}
if($row['p15'] != ""){$dop .= " -tickrate '".$tick."'";}
$dop .= " +fps_max 1000 +sv_lan 0 -insecure +tv_enable 1 +tv_maxclients 1  +tv_port ".$tv;
$run = 'cd /host/'.$user.'/'.$id.'/;chmod 777 srcds_run;screen -dmS server_'.$id.' sudo -u s'.$id.' ./srcds_run  
-console -game cstrike  -debug +servercfgfile server.cfg +map \''.$map.'\' +maxplayers '.$slots.' -ip '.$ip2.' -port '.$port2.' '.$dop.';';
$ssh->exec_cmd($run);
sleep('2');
$ssh->exec_cmd("ps -ef | grep srcds_i686  | grep -v grep | grep -v sh | grep ".$ip2." | grep ".$port2." | awk '{ print $2}';");
unset($data);
$data = $ssh->get_output();
$data = str_replace("\n","",$data);
$cpu = $slots*$conf['cpu_cssold'];
$ssh->exec_cmd('screen -dmS server_cpu_'.$id.' cpulimit -v -z -p '.$data.' -l '.$cpu.';');
            */
            ...
            run += $" ./srcds_run -console -game cstrike -debug +servercfgfile server.cfg +map '{map}' +maxplayers {slots}";
            run += $" -ip {ip} -port {port} {drop};";
            Client.RunCommand(run);
            Thread.Sleep(2000);
            run = $"ps -ef | grep srcds_i686 | grep -v grep | grep -v sh | grep {ip} | grep {port} | awk" + " '{ print $2}';";
            var result = Client.RunCommand(run);
            var data = result.Result.Replace('\n', ' ');
            var cpu = slots * cpu_cssold;
	...
	$data = $ssh->get_output();
	$data = str_replace("\n","",$data);
	$scan2 = "ps -ef | grep srcds_i686 | grep -v grep | grep -v sh | grep s".$id." | awk '{ print $2}'";
	...
	$ssh->exec_cmd("kill ".$data2.";kill ".$data.";kill ".$data3.";screen -wipe");
            */
            ...
            var result = Client.RunCommand(run);
            var data = result.Result.Replace('\n', ' ');
            var scan2 = $"ps -ef | grep srcds_i686 | grep -v grep | grep -v sh | grep s{id} | awk '" + "{ print $2}'";
